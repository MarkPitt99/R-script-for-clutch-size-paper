...
##
##
##### analysis of mass day 12 #####
##
##
mass_d12_model_full <-lmer(WEIGHT~
                          ...
                          data = data)
# simulate residuals 
weight_d12_model_full_residuals <- simulateResiduals(mass_d12_model_full, n = 1000, plot = T) # overall not too bad
testResiduals(weight_d12_model_full_residuals, plot = T)

# model results
summary(mass_d12_model_full)
drop1(mass_d12_model_full, test = 'Chisq')

# model without interactions
mass_d12_model <-lmer(WEIGHT~
                     ...
                     data = data)
# model results
summary(mass_d12_model)
drop1(mass_d12_model, test = 'Chisq')
...
## base table
weight_d12_table00 <- mass_d12_model %>%
...
## add features
weight_d12_table <- weight_d12_table00 %>% 
  ...
  modify_table_body(fun = function(.){
    output <- left_join(x = .,
                        y = drop1_output(x=mass_d12_model) %>% 
